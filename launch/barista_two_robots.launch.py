...
def generate_launch_description():
    ...
    world_file_arg = DeclareLaunchArgument(
        ...
        default_value=[get_package_share_directory(
            'barista_robot_description'), '/worlds/empty.world'],
   ...
   # Include the Gazebo launch file with the modified launch arguments
    ...
    spawn_robot2 = Node(
        package='gazebo_ros',
        executable='spawn_entity.py',
        arguments=['-entity', 'robot2', '-x', '1.0', '-y', '0.0', '-z', '0.2',
                   '-topic', robot_name_2+'/robot_description']
    )

    #RVIZ
    rviz_config_dir = os.path.join(get_package_share_directory(description_package_name), 'rviz', 'config.rviz')
    rviz_node = Node(
            package='rviz2',
            executable='rviz2',
            output='screen',
